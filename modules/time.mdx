---
title: 'TIME: Hours, Expenses, and Labour Compliance'
sidebarTitle: 'Time'
description: 'CyberOS TIME tracks billable hours, expenses, and leave with Vietnamese labour-law guardrails, VND expense OCR, and a three-tier approval flow.'
---
...
TIME supports three entry modes. Choose the mode that fits how you work; all three produce the same auditable `TimeEntry` row.
...
<Tabs>
  <Tab title="Timer">
    ...
    <Steps>
      <Step title="Start the timer">
        Open TIME and click **Start Timer**. Select the Engagement, Project, and Issue you are working on. The timer begins immediately and is stored in Redis so it survives page refreshes.
      </Step>
      <Step title="Work">
      ...
      <Step title="Stop the timer">
        Click **Stop Timer** and add a short description of what you did. CyberOS computes the `duration_minutes` from `start_time` and `end_time` as a generated column — you never type a duration.
      </Step>
    </Steps>
  </Tab>
  <Tab title="Manual entry">
    ...
    Open TIME, click **Log Time**, and enter the date, duration, Engagement, Project, Issue, and a description. The entry is created with `status = draft` and is included in your next weekly submission.
  </Tab>
...
Every week you submit your time entries for review. The flow has three tiers, and each step writes an audit row to the memory chain.

<Steps>
  <Step title="Member submits weekly">
    Before the end of each work week (Sunday 23:59 at the latest), select **Submit Week** in TIME. The system validates that there are no overlapping entries and no single entry longer than 12 continuous hours, then locks the entries with `status = submitted` and notifies your Account Manager.
  </Step>
...
<Note>
  You cannot approve your own time entries. The system rejects any approval attempt where the approver ID matches the member ID on the entry.
</Note>
...
<Steps>
  <Step title="Upload the receipt">
    In TIME, open **Expenses → Capture Expense** and upload or photograph the receipt. CyberOS stores the image in KMS-encrypted S3 and queues an OCR job immediately.
  </Step>
  <Step title="OCR processes the receipt">
    The OCR pipeline (AWS Textract at P1) parses the `hóa đơn` structure: vendor name, MST tax code, line items, VAT split, and total. The `vietnam-mst-validate` skill cross-checks the vendor's MST against the GDT registry. The categoriser assigns a chart-of-accounts code (for example, `T&E.meals` or `T&E.travel.taxi`) with a confidence score.
  </Step>
...
TIME supports **VND and USD** expenses. Every expense row stores the amount in its original currency plus a Vietcombank FX rate snapshot taken on the expense date, so the VND equivalent on your invoice is always reproducible.
...
<Warning>
  The system will block time entries that would breach the Vietnamese overtime cap. Attempts to exceed 300 hours of annual overtime are blocked at the API level and immediately page the CHRO. There are no exceptions — this is a hard rule, not a warning.
</Warning>
...
TIME entries are **append-only** at the audit layer. You can never edit an existing entry in place. If you need to correct a submitted or approved entry, TIME creates a new row with a `correction_to` link pointing at the superseded row and transitions the original to `status = corrected`.

```
Original entry A  (status=submitted)
       │
       └─── Correction B  (correction_to=A, status=submitted)
                 │
                 └─── AM approves B → status=approved
                      A → status=corrected
```

INV and all downstream consumers read `WHERE correction_to IS NULL AND status = 'approved'` — they always see only the latest approved entry in each correction chain, never the superseded original.
...
<Steps>
  <Step title="Rollup trigger">
    The weekly cron (Monday 10:00) or a manual Account Manager trigger initiates the rollup for the closed week or billing period.
  </Step>
  <Step title="Billable hours aggregated">
    TIME queries all approved entries where `billable = true` and `correction_to IS NULL`, grouped by Engagement, member, and role. The query applies the rate-card snapshot stored on each entry — not the current rate card — so retroactive rate changes never affect closed periods.
  </Step>
  <Step title="Summary pushed to INV">
    TIME pushes the rollup to INV with each line item containing Engagement ID, member, role, hours, rate-card snapshot, and VND total. INV creates a draft invoice for the Account Manager to review before sending to the client.
  </Step>
</Steps>
