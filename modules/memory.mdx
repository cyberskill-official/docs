---
title: 'BRAIN Memory: Audit-Chained Knowledge Store'
sidebarTitle: 'Memory (BRAIN)'
description: 'CyberOS BRAIN is an append-only, cryptographically chained memory store. Every write is verifiable; every fact is traceable back to the exact operation that created it.'
---
...
└── memories/
    ...
    ├── facts/               # grounded facts about the world
    ├── people/              # team members, contacts
    ├── projects/            # project-scoped context
    ├── preferences/         # user and agent preferences
    ├── drift/               # detected drift signals
    └── refinements/         # proposed refinements awaiting review
```
...
Layer 2 is a Rust service (`services/memory`) that reads Layer 1 and builds derived structure on top of it:

<CardGroup cols={3}>
  <Card title="Embeddings" icon="vector-square">
    Per-event embeddings stored in pgvector, enabling semantic recall over every memory the store has ever recorded.
  </Card>
...
Access to the Layer 2 service is consent-gated and tenant-scoped with fail-closed row-level security. The DB-backed test suite runs via `scripts/local_verify.sh`.
