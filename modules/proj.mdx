---
title: 'PROJ: Projects, Engagements, and Issue Tracking'
sidebarTitle: 'Projects'
description: 'CyberOS PROJ manages client engagements, projects, and issues with billable-hours tracking, rate cards, and the CRM deal-to-engagement bridge.'
---
...
<Steps>
  ...
  <Step title="Click Convert to Engagement">
    Select **Convert to Engagement** from the deal actions menu. The PROJ §2.5 join contract fires automatically: the Engagement is created with the rate card pre-populated from the deal's pricing page, and the deal's contract URL and client `account_id` are linked.
  </Step>
  ...
  <Step title="Review the rate card">
    Confirm the per-role rates (VND and USD) and the `billable_default` for each role. Adjust if the deal pricing differs from your default rate card.
  </Step>
  <Step title="Save">
...
Every time entry in TIME that references a PROJ Issue goes through a four-step cascade to determine whether it is billable. The decision is snapshotted on the `TimeEntry` row at write time — retroactive rate-card changes never shift past entries.
...
<Steps>
  ...
  <Step title="Step 2 — Task class (non-billable categories)">
    Each Engagement carries a list of `non_billable_categories` (for example, `internal-admin`, `pre-sales`, `travel`). If the Issue's category matches one of these, the entry is non-billable regardless of role.
  </Step>
  <Step title="Step 3 — Role default">
    The rate card on the Engagement specifies a `billable_default` per role (for example, `architect: true`, `intern: false`). If neither Step 1 nor Step 2 applies, the member's role default is used.
  </Step>
...
<Warning>
  Changing `non_billable_categories` on a live Engagement affects only new time entries. Past entries retain their snapshotted billable flag. If you need to retroactively correct an entry's billable status, use TIME's correction model to create a new corrected entry.
</Warning>
...
A rate card defines per-role pricing on an Engagement. Each role entry specifies a VND rate, an optional USD rate, and a `billable_default` flag.
...
<CardGroup cols={3}>
  ...
  <Card title="TIME" icon="clock">
    Members log time against PROJ Issues. Each entry goes through the billable cascade, and the decision is snapshotted. At billing cycle close, TIME rolls up approved billable hours per member per role per Engagement.
  </Card>
...
The rollup event carries rate-card snapshots, member overrides, and approval-flow flags. INV draws the draft invoice; the Account Manager reviews it; the client receives the final bill. Every invoice line is auditable to a specific chained `TimeEntry` row.
...
PROJ connects to five other CyberOS modules through explicit join contracts. Each integration is auditable — every cross-module event writes a chained memory row.

| Module | Direction | What flows |
|--------|-----------|------------|
| **CRM** | CRM → PROJ | Won deal triggers Engagement creation via the deal bridge. Rate card and client `account_id` are pre-wired. |
