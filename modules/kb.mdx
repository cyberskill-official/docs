---
title: 'KB: Documentation, Search, and AI Grounding'
...
Every save produces a new **immutable version**. The document row's `current_version_id` pointer advances to the new version; prior versions are never modified. Version rows are chained in the memory audit chain — you can always reconstruct exactly what a document said at any point in time.

<Note>
  There is no "edit in place" in KB. Every keystroke that results in a save produces a new version row. This is intentional: KB documents are cited by AI answers and referenced by PROJ Issues. If the underlying content could change silently, those citations would become untrustworthy.
</Note>
...
<Steps>
  ...
  <Step title="Write in markdown">
    Use the built-in editor (or any markdown tool via the REST API). Include YAML frontmatter at the top of the file. Attachments (images, PDFs) upload directly in the editor and are stored in KMS-encrypted S3.
  </Step>
  <Step title="Set permissions">
    Choose one of the permission tiers (see below) before publishing. You can tighten permissions later, but loosening them requires an explicit ACL change that writes an audit row.
  </Step>
  <Step title="Save and publish">
    Click **Save**. CyberOS renders the markdown to sanitised HTML, creates an immutable version row, and queues a memory re-ingestion job. The document is live and searchable within seconds.
  </Step>
</Steps>
...
| `role-restricted` | Members holding one of the `allowed_role_codes` | HR policy, compensation references, security runbooks |
...
<Steps>
  <Step title="Layer 1 — Lexical (FTS5 / PGroonga)">
    Full-text search using FTS5 for Latin-script content and PGroonga with Vietnamese bigram tokenisation for Vietnamese. This layer returns up to 100 candidate document versions based on keyword overlap. Vietnamese queries like *"hóa đơn cấp khi nào"* are tokenised correctly without needing the user to segment words manually.
  </Step>
  <Step title="Layer 2 — Semantic (BGE-M3 embeddings)">
  ...
  <Step title="Layer 3 — Reranker (BGE-rerank-v2-m3)">
...
<Note>
  ACL filtering happens **before** the reranker — documents you are not permitted to read never reach the reranker, the QA composer, or the search results. An ACL leak via search is a severity-0 bug.
</Note>
...
<Steps>
  ...
  <Step title="KB retrieves relevant spans">
    KB embeds your question with BGE-M3, finds the top-8 most relevant text spans across the current document and its linked documents, and passes them to the QA composer.
  </Step>
  <Step title="AI composes a cited answer">
    The QA composer calls the AI Gateway with a system instruction requiring every claim to cite a span ID. The AI returns a structured answer with inline citations.
  </Step>
...
Every time you save a document, KB automatically re-indexes it to memory Layer 2 within **5 seconds p95**. The ingestion pipeline:
...
4. Upserts the chunks and vectors into memory Layer 2
5. Writes an audit row confirming the ingestion latency
...
**Promote to canonical** elevates a document from a regular knowledge base page to a high-authority memory source used by Lumi cross-tenant AI synthesis (where `sync_class` permits).

<Steps>
  <Step title="Request promotion">
    Open the document and click **Promote to canonical**. Enter the memory canonical path (for example, `memories/policy/leave.md`). This triggers a CHAT notification to the CDO seat for approval.
  </Step>
  <Step title="CDO approves">
    The CDO reviews the document to confirm it represents authoritative, stable knowledge suitable for cross-tenant synthesis. Approval is logged as an audit row.
  </Step>
  <Step title="Canonical entry created">
    KB writes a Layer 1 canonical memory entry mirroring the document. The document is now a high-authority source — AI answers that cite it carry higher confidence weighting than answers citing ordinary Layer 2 chunks.
  </Step>
</Steps>
