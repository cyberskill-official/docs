---
title: 'PORTAL: White-Label Client Portal for Agencies'
sidebarTitle: 'Portal'
...
| Layer | How it works |
|---|---|
| **Postgres RLS** | Every portal-related table has row-level security enforced by `(tenant_id, client_account_id)`. A misconfigured query cannot return another client's rows. |
| **Apollo Router scope checks** | Every GraphQL field requires a `portal.read` scope grant bound to the client's account. The router injects the `client_account_id` predicate into every query before it reaches the subgraph. |
