---
title: 'LEARN: Skills Catalogue and Career Growth'
sidebarTitle: 'LEARN'
...
<Warning>
  Per-judge individual scores **never leave LEARN**. This is the psychological-safety contract that makes honest peer review possible. Do not attempt to retrieve per-judge data via the admin REST API — the `/admin/vault/{case_id}` endpoint is restricted to the LEARN admin role and is explicitly blocked for HR's read role.
</Warning>
