---
title: 'CRM: Sales Pipeline, Accounts, and Vietnamese Commerce'
sidebarTitle: 'CRM'
description: 'CyberOS CRM manages your sales pipeline with Account, Contact, and Deal — plus Vietnamese tax-code validation, VietQR payment, and hóa đơn.'
---
...
<Tabs>
  ...
  <Tab title="Next-best-action">
    CUO ranks the single most valuable next move for each open Deal. Suggestions include **call**, **send proposal**, **schedule demo**, and **request VietQR collection**. Each suggestion shows a confidence percentage and the reasoning behind it, grounded in prior interactions with the account and similar deal trajectories.
  </Tab>
  ...
  <Tab title="Win/loss analysis">
    After a deal is closed (won or lost), CUO drafts a win/loss analysis grounded in the deal's activity history, competitor signals, and similar past deals. The Account Manager reviews and accepts the draft, which is then stored as a knowledge record that future deals with the same account or vertical can reference.
  </Tab>
</Tabs>
...
<Steps>
  ...
  <Step title="Deal record archived">
...
| Hóa đơn emission | **Automatic** on deal closed-won via INV; **manual confirm** required | Tax document — the AM must confirm amount and recipient before emission. |
| Deal → Engagement conversion | **Manual** AM click | Not every won deal becomes a PROJ Engagement; AM intent is required. |
| Win/loss analysis | **Automatic** draft via CUO; **AM accepts** | Generates a knowledge record for future deals to cite; the AM owns the narrative. |
