---
title: 'Core Concepts: How CyberOS Works'
sidebarTitle: 'Concepts'
description: 'Understand the CyberOS model: BRAIN memory, Skills, CUO workflows, tasks, human-in-the-loop gates, and the .cyberos/ module tree.'
---
...
Run `python -m cyberos doctor` at any time to walk the full invariant set and confirm store health. Run `cyberos export` to produce a deterministic, portable zip of the entire store.

### Layer 2 — the brain service

`services/memory` is a Rust service that reads Layer 1 and derives higher-level structure from it:

- **Per-event embeddings** stored in pgvector for semantic similarity search.
- **Rolling summaries** that compress long histories into retrievable snapshots.
- **Hot/warm/cold tiering** keyed on each interaction's `occurred_at` timestamp — recent memories are fast to retrieve; older memories are tiered but still provable.
