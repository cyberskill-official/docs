...
- **Hook trạng thái mồ côi không còn chặn commit.** Hook `pre-commit` còn sót lại từng làm hỏng mọi commit chạm vào `VERSION`, `CHANGELOG.md` hoặc `docs/tasks/` nếu bạn xóa thư mục `.cyberos/` của repo mà không chạy `cyberos uninstall`. Hook giờ đây cảnh báo hai lần, nêu tên hai cách khắc phục và thoát sạch. Các máy có bản cài đặt còn nguyên không bị ảnh hưởng.
- **Pipeline phát hành đã được xác minh toàn diện.** Checklist phát hành 1.0.0 đã chạy đến hoàn thành (20/20). Luồng tag → build → publish, release notes trên GitHub và trang status trên site tài liệu đều render từ cùng một corpus. Trang [changelog](/vi/reference/changelog) hiện là nguồn chân lý cho trạng thái đã ship.

### Cập nhật

- **Uninstall giờ đây là nghịch đảo của install.** `cyberos uninstall` gỡ các đăng ký MCP của chính nó, dọn sạch mọi liên kết skill được quản lý trên các họ Claude, Grok, Codex, OpenCode và Cursor, và strip byte-chính-xác các mục nó đã thêm vào `.gitignore` và pre-commit hook. Các tệp của người vận hành mà nó chưa từng ghi được bảo toàn. Xem [Cài đặt](/vi/guides/install).
