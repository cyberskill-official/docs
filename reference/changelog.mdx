...
<Tabs>
  <Tab title="Highlights">
    ...
    - **Consumer CLI finalised.** Eight `npx cyberos <command>` entry points: `install`, `uninstall`, `version`, `status`, `help`, plus the CLI-only `create` (scaffold a new repo), `gates` (run machine gates), and `mcp` (launch the stdio MCP server). Matching Claude plugin slash commands: `/install`, `/uninstall`, `/version`, `/status`, `/help`, alongside the plugin-only task-workflow commands `/ship-tasks`, `/plan`, `/create-tasks`, and `/improve`.
...
<AccordionGroup>
  ...
  <Accordion title="Claude Plugin — Commands and Marketplace">
    - **CLI-parity slash commands:** `/install`, `/uninstall`, `/version`, `/status`, `/help` — full parity with the `npx cyberos` CLI surface.
    - **Task-workflow slash commands:** `/ship-tasks`, `/plan`, `/create-tasks`, `/improve` — drive the CUO task workflow end to end. Plugin-only; no CLI equivalent.
