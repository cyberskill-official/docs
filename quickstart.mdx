...
<Steps>
  ...
  <Step title="Trigger the agent">
    ...
    | `/plan` | Draft a plan for the next work bucket before opening tasks |
    | `/create-tasks` | Turn a plan or requirement into task files under `docs/tasks/` |
    | `/improve` | Open an improvement task against CyberOS itself |
    | `/install` | Install or re-vendor CyberOS into the current repo |
    | `/uninstall` | Remove the vendored `.cyberos/` folder from the current repo |
    | `/version` | Check whether a newer CyberOS is available |
    | `/status` | Open the board / table / releases status page in the browser |
...
```bash npx version
# Check whether a newer CyberOS is available for the current directory
npx cyberos version

# Check a specific repo
npx cyberos version /path/to/your-repo

# If stale, re-vendor with the latest payload
npx cyberos install
```
